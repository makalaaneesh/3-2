...
void dump_basic_info(const u_char * packet){
    ...
    printf("[%d] ", (unsigned int)iph->protocol);
    char ipstr[INET6_ADDRSTRLEN];
    inet_ntop(AF_INET, &source.sin_addr, ipstr, sizeof ipstr);
    printf("%s:%u -> ", ipstr, ntohs(tcph->source));
	...
	const char *data = packet + sizeof(struct ethhdr) + iphdrlen + tcphdrlen;
	printf("%s\n", data);
	printf("-----------------------------------------------------\n");
...
void open_device(int argc, char **argv){
	/*
