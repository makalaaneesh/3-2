...
int current_registered_clients = 0;
int registered_client_fds[MAX_CLIENTS];
...
void *run(void * arg){
	...
	// Thread has started. So we can register the client
	registered_client_fds[current_registered_clients++] = sfd;
	...
	while(1){
		...
		// send back message to all the registered clients
		for(i = 0; i< current_registered_clients; i++){
			int client_sfd = registered_client_fds[i];
			int s = send(client_sfd, readbuffer, strlen(readbuffer), 0);
...
int main(int argc, char *argv[]){
	...
	printf("%s\n", "Group is listening for connections.");
	...
	while(1){
		...
		if (current_no_of_clients >= MAX_CLIENTS){
			...
			printf("Connectinf\n");
			strcpy(writebuffer, "Welcome to the group chat .service!");
			// int s = send(sfd, writebuffer, strlen(writebuffer), 0);
			// print_error(s, "Failed to send message to the client.");
			memset(writebuffer, 0, strlen(writebuffer));

			pthread_create(&(threads[current_no_of_clients++]), NULL, run, (void*)(&nsfd));
			current_no_of_clients++;
		}
